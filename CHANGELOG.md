...
- Upgrades server version, including the API break that came with it: NPC behaviour hooks (actions/sensors) now receive an `ExecutionSupport` instead of a `Role`, commands declare open access via `requireNoPermission()` instead of the removed `canGeneratePermission()`, and block lookups moved to the new chunk-section storage. Block-entity world positions are now resolved through the engine itself, which fixes Workstation block entities being located at the wrong height above the lowest chunk section.
