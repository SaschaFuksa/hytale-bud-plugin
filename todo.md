...
**Nachtrag:** Die erste Verifikationsrunde (No-Op-Touch weit entfernt von der eigentlichen Änderung, z. B. an der `package`-Zeile) hat sich als unzuverlässig herausgestellt — der Sprachserver liefert darüber offenbar nur Diagnostics für den unmittelbar editierten Bereich zurück, nicht zwingend einen vollständigen Datei-Sweep. Dadurch initial übersehen und vom Nutzer aus dem echten VS-Code-Problems-Panel nachgemeldet:

- [x] `LLMInteractionManager.java:91` — verschachtelt in der bereits gefixten `Message.join(...)`-Zeile: `trimmedMessage = message.trim()` ist selbst wieder ein unannotierter `String.trim()`-Rückgabewert, der als `@Nonnull`-Argument an `Message.raw(...)` ging. Fix direkt an der Deklaration: `Objects.requireNonNull(message.trim())`.
- [x] `WeatherEntry.java:15` — Record-Compact-Constructor-Eigenheit: `javax.annotation.Nonnull` ist nicht `TYPE_USE`, daher propagiert die `@Nonnull`-Annotation vom Record-Header nicht zuverlässig auf die implizite Zuweisung/Nutzung der Komponente *innerhalb* des Compact-Constructor-Bodys — betraf nicht nur die bereits gefixte `weatherName`-Zuweisung selbst, sondern auch die Übergabe von `timeOfDay` als Argument an `WeatherInterpreter.resolveDisplayName(...)`. Fix: zusätzlich `Objects.requireNonNull(timeOfDay)` an der Verwendungsstelle.

Danach gezielt geprüft, ob `WeatherEntry` der einzige Record mit Compact Constructor im Projekt ist (`grep -rn "public \w+Entry {|public \w+Event {"` → ja, nur dieser eine), damit diese Fehlerklasse nicht anderswo unentdeckt bleibt.

- [x] Problems-Panel leer (laut Nutzer-Rückmeldung nach Fix der beiden nachgemeldeten Stellen; keine neuen `@SuppressWarnings` hinzugefügt — `grep -rn "@SuppressWarnings" src/main/java` zeigt nur die zwei vor dieser Phase bereits vorhandenen, unabhängigen Stellen in `BudPlugin.java` und `RecentItemCache.java`)
- [x] `.\gradlew build` grün (`.\gradlew build` erneut erfolgreich nach den beiden Nachträgen)
