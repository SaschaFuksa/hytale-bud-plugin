...
- [x] **Karte eingelegt** → Reaktion beim Arbeitsbeginn. `WorkstationBindingHandler.performBind(...)` feuert einen `WorkEntry(START)` auf dem `ACTIVITY`-Kanal, bud-spezifisch über neues `workView`/`workViewStart` je Bud-YAML.
- [x] **Karte entnommen** → Reaktion beim Feierabend, vor dem Despawn ausgelöst. Orchestrator-Queue bewusst umgangen (Timing-Race, siehe `docs/bud-worker-mode-plan.md`, "Phase 9") — `despawnBoundBud(...)` ruft die Reaktion direkt auf einem eigenen virtuellen Thread auf und plant den Despawn erst im `finally`-Block danach.
- [x] **Ansprechen mit F während der Arbeit** → neue NPC-Action `WorkTalk`, eingehängt in den bestehenden `InteractionInstruction`/`HasInteracted`-Cycle aller drei Rollen-JSONs (kein Zustandswechsel), `PLAYER`-Kanal ohne Cooldown.
- [x] ~~Periodischer Arbeitskommentar~~ — **umgesetzt, dann auf Saschas Wunsch (2026-08-14) wieder vollständig entfernt.** Begründung: redundant zum bereits bestehenden allgemeinen Dialog-Modus, der ohnehin gelegentlich von sich aus startet. Vollständig entfernt: `WorkCommentTracker` (ganze Klasse), der Aufruf in `PlayerJoinSystem`, `WorkReactionKind.PERIODIC`, `BudManager.getRandomWorkingBudComponent(...)` (war ausschließlich für den Tracker gebaut, gegengeprüft dass kein anderer Aufrufer existierte), `WorkConfig.WorkCommentIntervalSeconds` (Feld/Getter/Codec/README-Zeile), `system_prompt.yml`s `workComment`-Eintrag, `workViewComment` aus allen drei Bud-YAMLs. Per `grep` verifiziert, dass keine Fundstelle übrig blieb.
- [x] **Neu dafür: Output-voll-Reaktion.** Wenn eine reife Kachel wegen vollem Output gesperrt wird (`WorkstationFuelTickSystem`, `hasHarvestOutputRoom(...) == false`), feuert der gebundene Bud **einmalig** eine `WorkEntry(OUTPUT_FULL)`-Reaktion (`ACTIVITY`-Kanal, Bud steht über `workstation.getBoundBud()` schon fest, keine Zufallsauswahl nötig) — neues `WorkstationBlockEntity.outputFullReactionSent`-Flag verhindert Wiederholung bei jeder erneuten Prüfung. Zurückgesetzt über den bereits bestehenden Output-Container-Change-Listener in `WorkstationFilterSystem` (umbenannt zu `workstation.onOutputContainerChanged()`, erledigt Lock-Clear und Flag-Reset in einem Aufruf). Die LLM-Variante nennt die konkrete blockierende Sorte (`WorkEntry.blockedItemId`, aufgelöst über neues `resolveHarvestItemId(...)`, gleiches Prinzip wie `workType`/`describeWorkType`); der Fallback-Text bleibt bewusst generisch statisch (`workViewOutputFull` je Bud-YAML), da im gesamten Projekt kein Fallback-Text dynamische Inhalte interpoliert — Konsistenz mit dem bestehenden Muster war hier wichtiger als die zusätzliche Konkretheit auch im Non-LLM-Fall.

**Umsetzung:** siehe `docs/bud-worker-mode-plan.md`, Abschnitt "Phase 9 — LLM-Reaktionen im Arbeitsmodus" für die vollständige Begründung (Working-Ausschluss-Umgehung, Fallback-Text, Reihenfolge-Fix bei Anlass 2, Entscheidung gegen `PlayerInteractEvent` bei Anlass 3, Streichung/Ersatz des periodischen Kommentars). Paket `com.bud.feature.work.reaction` (`WorkReactionKind`: START/END/INTERACT/OUTPUT_FULL, `WorkEntry`, `LLMWorkMessageCreation`, `WorkTalkAction`/`BuilderActionWorkTalk`), neue `IQueueEntry.allowsReactionWhileWorking()` (Default `false`, nur `WorkEntry` überschreibt sie), `Orchestrator.dispatch` respektiert sie zusätzlich zum bisherigen `WORKING`-Check. `ReactionConfig.EnableWorkReactions` (Default `true`) gated alle drei verbliebenen Anlässe gemeinsam. `promptVersion` in `versions.yml` hochgezählt. `.\gradlew build` nach jedem der beiden Punkte einzeln grün.
- [x] **Ingame-Test bestätigt (Sascha, 2026-08-15):** alle Anlässe funktionieren — Karte rein/raus, F-Ansprache während der Arbeit, Output-voll-Reaktion mit Sortennamen und korrektem Reset bei Entnahme. Phase 9 damit abgeschlossen.
...
## Kosmetische Politur (Farming, vor Foresting-Start — Sascha, 2026-08-14)

Eigenes Stationsmodell ist bereits erledigt (Sascha, Asset-seitig). Sammelpunkt für den restlichen kosmetischen Feinschliff an Keyleths Farming, bevor mit Gronkh/Foresting begonnen wird — Liste wird ergänzt, sobald Sascha beim Spielen weitere Punkte findet, kein Anspruch auf Vollständigkeit jetzt schon.

- [x] **Eigenes Stationsmodell** (Mini-Farm-Optik statt Furnace-Platzhalter) — Sascha, erledigt.
- [ ] **Charakterspezifische Ruhepose für Keyleth** (`Sit`/`Sit_Down`/`Sit_Up` statt generischem `Crouch` im Futter-leer-Zustand) — aus Phase 6 zurückgestellt, siehe dortige Begründung (Sub-State-Mechanismus `.Resting` noch nicht bytecode-verifiziert). Jetzt fällig laut eigener damaliger Notiz ("vor Phase 9/10 aufgreifen").
- [ ] **Slot-Icons** für Karten-/Saatgut-Slot (aktuell generische Icons?) — Status seit dem Bench-Umbau (Phase 3b) noch nicht gegengeprüft, muss zuerst geklärt werden ob überhaupt noch offen.
